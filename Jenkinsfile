        ...
        stage('Schedule or Manual') {
            steps {
                script {
                    ...
                    if (isScheduled) {
                        currentBuild.displayName = "Schedule Run (dev-269, dev-270) (#${env.BUILD_NUMBER})"
                        ['269', '270'].each { devNum ->
                            def branchName = (devNum == '269') ? 'develop/test' : 'main'
        ...
        always {
            script {
                ...
                if (!isScheduled) {
                    ...
                    sh 'python3 parse_robot_results.py results1/output.xml results1/test_summary.txt'
                    // Show parser output
                    sh 'cat results1/test_summary.txt'
                    // Zip the report directory
                    sh 'cd results1 && zip -r report.zip .'

                    // Read summary from file and send email using CPS-safe readJSON
                    def summary = readJSON file: 'results1/test_summary.txt'
